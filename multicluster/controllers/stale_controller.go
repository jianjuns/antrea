...
type StaleResCleanupController struct {
	...
	commonAreaGetter member.RemoteCommonAreaGetter
	namespace        string
	clusterRole      string
...
func (c *StaleResCleanupController) cleanupStaleResourcesOnMember() error {
	...
	commonArea, c.localClusterID, err = GetRemoteCommonAreaAndLocalID()
	if err != nil {
		return err
	}

	resImpList := &mcsv1alpha1.ResourceImportList{}
	if err := commonArea.List(common.ctx, resImpList, &client.ListOptions{Namespace: commonArea.GetNamespace()}); err != nil {
		return err
	}
	...
	if err := commonArea.List(common.ctx, resExpList, &client.ListOptions{Namespace: commonArea.GetNamespace()}); err != nil {
		return err
	}
...
func (c *StaleResCleanupController) cleanupStaleServiceResources(commonArea commonarea.RemoteCommonArea,
	...
	if err := c.List(common.ctx, svcImpList, &client.ListOptions{}); err != nil {
		return err
	}

	svcList := &corev1.ServiceList{}
	if err := c.List(common.ctx, svcList, &client.ListOptions{}); err != nil {
		return err
	}
	...
	for _, staleSvc := range mcsSvcItems {
		...
		if err := c.Client.Delete(common.ctx, &svc, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}
	for _, staleSvcImp := range svcImpItems {
		...
		if err := c.Client.Delete(common.ctx, &svcImp, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (c *StaleResCleanupController) cleanupACNPResources(resImpList *mcsv1alpha1.ResourceImportList) error {
	...
	if err := c.List(common.ctx, acnpList, &client.ListOptions{}); err != nil {
		return err
	}
	...
	for _, stalePolicy := range staleMCACNPItems {
		...
		if err := c.Client.Delete(common.ctx, &acnp, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (c *StaleResCleanupController) cleanupClusterInfoImport(resImpList *mcsv1alpha1.ResourceImportList) error {
	...
	if err := c.List(common.ctx, ciImpList, &client.ListOptions{}); err != nil {
		return err
	}
	...
	for _, staleCIImp := range staleCIImps {
		...
		if err := c.Client.Delete(common.ctx, &ciImp, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (c *StaleResCleanupController) cleanupLabelIdentities(resImpList *mcsv1alpha1.ResourceImportList) error {
	...
	if err := c.List(common.ctx, labelIdentityList, &client.ListOptions{}); err != nil {
		return err
	}
	...
	for _, l := range staleLabelIdentities {
		...
		if err := c.Client.Delete(common.ctx, &labelIdentity, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (c *StaleResCleanupController) cleanupServiceResourceExport(commonArea commonarea.RemoteCommonArea,
	...
	if err := c.List(common.ctx, svcExpList, &client.ListOptions{}); err != nil {
		return err
	}
	...
	for _, r := range staleResExpItems {
		...
		if err := commonArea.Delete(common.ctx, &re, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (c *StaleResCleanupController) cleanupLabelIdentityResourceExport(commonArea commonarea.RemoteCommonArea,
	...
	if err := c.List(common.ctx, podList, &client.ListOptions{}); err != nil {
		return err
	}
	if err := c.List(common.ctx, nsList, &client.ListOptions{}); err != nil {
		return err
	}
	allResExpItems := resExpList.Items
	staleResExpItems := map[string]mcsv1alpha1.ResourceExport{}
	for _, resExp := range allResExpItems {
		if resExp.Spec.Kind == common.LabelIdentityKind && resExp.Labels[common.SourceClusterID] == c.localClusterID {
...
func (c *StaleResCleanupController) cleanupClusterInfoResourceExport(commonArea commonarea.RemoteCommonArea,
	...
	if err := c.Client.List(common.ctx, &gws, &client.ListOptions{}); err != nil {
		return err
	}
	...
	if len(gws.Items) == 0 {
		ciExport := &mcsv1alpha1.ResourceExport{
			ObjectMeta: metav1.ObjectMeta{
				...
				Name:      member.newClusterInfoResourceExportName(c.localClusterID),
			},
		}
		klog.InfoS("Cleaning up stale ClusterInfo kind of ResourceExport", "resourceexport", klog.KObj(ciExport))
		if err := commonArea.Delete(common.ctx, ciExport, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
			return err
		}
	}
	return nil
...
func (c *StaleResCleanupController) cleanupMemberClusterAnnounces() error {
	...
	if err := c.List(common.ctx, memberClusterAnnounceList, &client.ListOptions{}); err != nil {
		return err
	}
	...
	for _, m := range memberClusterAnnounceList.Items {
		...
		if err := c.Client.Delete(common.ctx, &memberClusterAnnounce, &client.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
