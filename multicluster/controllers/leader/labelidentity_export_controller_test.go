...
var (
	...
	resExpNamespacedName = types.NamespacedName{
		...
		Name:      controllers.localClusterID + "-" + labelHash,
	}
	...
	labelIdentityResImp = &mcsv1alpha1.ResourceImport{
		...
		Spec: mcsv1alpha1.ResourceImportSpec{
			ClusterIDs: []string{controllers.localClusterID},
...
func TestLabelIdentityResourceExportReconclie(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expLabelsToClusters:   map[string]sets.String{labelHash: sets.NewString(controllers.localClusterID)},
			expClusterToLabels:    map[string]sets.String{controllers.localClusterID: sets.NewString(labelHash)},
		...
		},
		{
			...
			expNormalizedLabel:       normalizedLabel,
			originalLabelsToClusters: map[string]sets.String{labelHash: sets.NewString(controllers.localClusterID, clusterBID)},
			originalClusterToLabels:  map[string]sets.String{controllers.localClusterID: sets.NewString(labelHash), clusterBID: sets.NewString(labelHash)},
			expLabelsToClusters:      map[string]sets.String{labelHash: sets.NewString(controllers.localClusterID)},
			expClusterToLabels:       map[string]sets.String{controllers.localClusterID: sets.NewString(labelHash), clusterBID: sets.NewString()},
			expLabelResImpDeleted:    false,
		},
		{
			...
			resExpNamespacedName:     resExpNamespacedName,
			expNormalizedLabel:       "",
			originalLabelsToClusters: map[string]sets.String{labelHash: sets.NewString(controllers.localClusterID)},
			originalClusterToLabels:  map[string]sets.String{controllers.localClusterID: sets.NewString(labelHash)},
			expLabelsToClusters:      map[string]sets.String{},
			expClusterToLabels:       map[string]sets.String{controllers.localClusterID: sets.NewString()},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithLists(tt.existResExp, tt.existResImp).Build()
			r := NewLabelIdentityExportReconciler(fakeClient, controllers.scheme, controllers.leaderNamespace)
			...
			if _, err := r.Reconcile(controllers.ctx, resExpReq); err != nil {
			...
			err := fakeClient.Get(controllers.ctx, types.NamespacedName{Namespace: "default", Name: parsedLabelHash}, actLabelIdentityResImp)
