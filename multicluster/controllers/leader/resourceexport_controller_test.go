...
func TestResourceExportReconciler_handleServiceExportDeleteEvent(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingResExport, existResImport).Build()
	r := NewResourceExportReconciler(fakeClient, controllers.scheme)
	if _, err := r.Reconcile(controllers.ctx, svcResReq); err != nil {
		...
		err := fakeClient.Get(controllers.ctx, namespacedName, resImport)
...
func TestResourceExportReconciler_handleEndpointsExportDeleteEvent(t *testing.T) {
	...
	existingResExport2 := &mcsv1alpha1.ResourceExport{
		...
		Spec: mcsv1alpha1.ResourceExportSpec{
			...
			Kind:      common.EndpointsKind,
			Endpoints: &mcsv1alpha1.EndpointsExport{
				Subsets: controllers.epNginxSubset2,
			},
		},
	}
	existResImport := &mcsv1alpha1.ResourceImport{
		...
		Spec: mcsv1alpha1.ResourceImportSpec{
			...
			Endpoints: &mcsv1alpha1.EndpointsImport{
				Subsets: append(controllers.epNginxSubset, controllers.epNginxSubset2...),
			},
		},
	}
	expectedSubsets := controllers.epNginxSubset2
	namespacedName := types.NamespacedName{Namespace: "default", Name: "default-nginx-endpoints"}
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingResExport1, existingResExport2, existResImport).Build()
	r := NewResourceExportReconciler(fakeClient, controllers.scheme)
	if _, err := r.Reconcile(controllers.ctx, epResReq); err != nil {
...
func TestResourceExportReconciler_handleServiceExportCreateEvent(t *testing.T) {
	existingResExport := &mcsv1alpha1.ResourceExport{
		...
		Spec: mcsv1alpha1.ResourceExportSpec{
			...
			Service: &mcsv1alpha1.ServiceExport{
				ServiceSpec: controllers.svcNginxSpec,
			},
		},
	}
	...
	namespacedName := types.NamespacedName{Namespace: "default", Name: "default-nginx-service"}
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingResExport).Build()
	r := NewResourceExportReconciler(fakeClient, controllers.scheme)
	if _, err := r.Reconcile(controllers.ctx, svcResReq); err != nil {
...
func TestResourceExportReconciler_handleEndpointExportCreateEvent(t *testing.T) {
	...
	namespacedName := types.NamespacedName{Namespace: "default", Name: "default-nginx-endpoints"}
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existEPResExport, existSvcResExport).Build()
	r := NewResourceExportReconciler(fakeClient, controllers.scheme)
	if _, err := r.Reconcile(controllers.ctx, epResReq); err != nil {
...
func TestResourceExportReconciler_handleACNPExportCreateEvent(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingResExport).Build()
	r := NewResourceExportReconciler(fakeClient, controllers.scheme)
	if _, err := r.Reconcile(controllers.ctx, acnpResReq); err != nil {
...
var (
		...
		Spec: mcsv1alpha1.ResourceExportSpec{
			...
			Service: &mcsv1alpha1.ServiceExport{
				ServiceSpec: corev1.ServiceSpec{
					...
					Ports:      []corev1.ServicePort{controllers.svcPort8080},
	...
	existResImport = &mcsv1alpha1.ResourceImport{
		...
		Spec: mcsv1alpha1.ResourceImportSpec{
			...
			ServiceImport: &mcs.ServiceImport{
				Spec: mcs.ServiceImportSpec{
					Ports: SvcPortsConverter([]corev1.ServicePort{controllers.svcPort80}),
					Type:  mcs.ClusterSetIP,
				},
			},
		},
...
func TestResourceExportReconciler_handleSingleServiceUpdateEvent(t *testing.T) {
	...
	namespacedName := types.NamespacedName{Namespace: "default", Name: "default-nginx-service"}
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).
...
func TestResourceExportReconciler_handleServiceUpdateEvent(t *testing.T) {
	...
	if _, err := r.Reconcile(controllers.ctx, svcResReq); err != nil {
		...
		err := fakeClient.Get(controllers.ctx, types.NamespacedName{Namespace: svcResReq.Namespace, Name: svcResReq.Name}, updatedSvcResExport)
		if err != nil {
...
func TestResourceExportReconciler_handleClusterInfoKind(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(&clusterACIResExport, &clusterBCIResExport,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := r.Reconcile(controllers.ctx, req); err != nil {
				...
				err := fakeClient.Get(controllers.ctx, namespacedName, &teImport)
				if err == nil {
					if tt.isDelete {
					...
					err := fakeClient.Get(controllers.ctx, namespacedName, &teExport)
					if !apierrors.IsNotFound(err) {
