...
func TestLabelIdentityReconciler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expNormalizedLabels:  []string{controllers.normalizedLabel},
			expLabelsToPodsCache: map[string]sets.String{controllers.normalizedLabel: sets.NewString(podAName)},
			expPodLabelCache:     map[string]string{podANamespacedName.String(): controllers.normalizedLabel},
		},
		{
			...
			expNormalizedLabels:  []string{controllers.normalizedLabel},
			expLabelsToPodsCache: map[string]sets.String{controllers.normalizedLabel: sets.NewString(podAName, podBName)},
			expPodLabelCache:     map[string]string{podAName: controllers.normalizedLabel, podBName: controllers.normalizedLabel},
		},
		{
		...
		},
		{
			...
			expNormalizedLabels:  []string{controllers.normalizedLabel},
			expLabelsToPodsCache: map[string]sets.String{controllers.normalizedLabel: sets.NewString(podAName)},
			expPodLabelCache:     map[string]string{podAName: controllers.normalizedLabel},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithLists(tt.existingPods).WithObjects(ns).Build()
			fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).Build()
			commonArea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader-cluster", controllers.localClusterID, controllers.leaderNamespace, nil)
			mcReconciler := NewMemberClusterSetReconciler(fakeClient, controllers.scheme, "default", true)
			mcReconciler.SetRemoteCommonArea(commonArea)
			r := NewLabelIdentityReconciler(fakeClient, controllers.scheme, mcReconciler)
			go r.Run(stopCh)
			...
			for _, p := range tt.existingPods.Items {
				...
				_, err := r.Reconcile(controllers.ctx, req)
			...
			case addEvent:
				...
				Client.Create(controllers.ctx, tt.podEvent, &client.CreateOptions{})
			case updateEvent:
				Client.Update(controllers.ctx, tt.podEvent, &client.UpdateOptions{})
			case deleteEvent:
				Client.Delete(controllers.ctx, tt.podEvent, &client.DeleteOptions{})
			}
			var err error
			...
			err = commonArea.List(controllers.ctx, actLabelIdentityResourceExports)
...
func TestNamespaceMapFunc(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(podA, podC, ns).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).Build()
	commonArea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader-cluster", controllers.localClusterID, controllers.leaderNamespace, nil)
	mcReconciler := NewMemberClusterSetReconciler(fakeClient, controllers.scheme, "default", true)
	mcReconciler.SetRemoteCommonArea(commonArea)
