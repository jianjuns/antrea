...
func (r *LabelIdentityReconciler) checkRemoteCommonArea() bool {
	...
	if r.remoteCommonArea == nil {
		commonArea, localClusterID, _ := GetRemoteCommonAreaAndLocalID()
		if commonArea == nil {
			return true
		}
...
func (r *LabelIdentityReconciler) syncLabelResourceExport(label string) error {
	...
	if exists {
		...
		if err := r.createLabelIdentityResExport(controllers.ctx, label); err != nil {
			return err
		}
	} else {
		if err := r.deleteLabelIdentityResExport(controllers.ctx, label); err != nil {
			return err
		}
	}
	return nil
