...
func TestNodeReconciler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := NewNodeReconciler(fakeClient, controllers.scheme, "default", tt.precedence)
			...
			if _, err := r.Reconcile(controllers.ctx, tt.req); err != nil {
				...
				err := fakeClient.Get(controllers.ctx, gwNamespcedName, newGW)
				if tt.isDelete {
...
func TestInitialize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(obj...).Build()
			r := NewNodeReconciler(fakeClient, controllers.scheme, "default", mcsv1alpha1.PrecedencePublic)
			if err := r.initialize(); err != nil {
				...
				if tt.isDelete {
					...
					err := fakeClient.Get(controllers.ctx, gwNamespcedName, deletedGW)
