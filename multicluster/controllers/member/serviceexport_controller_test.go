...
import (
	...
	"antrea.io/antrea/multicluster/controllers/commonarea"
)
...
func TestServiceExportReconciler_handleDeleteEvent(t *testing.T) {
	existSvcResExport := &mcsv1alpha1.ResourceExport{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: controllers.leaderNamespace,
			Name:      getResourceExportName(controllers.localClusterID, nginxReq, "service"),
		},
	}
	existEpResExport := &mcsv1alpha1.ResourceExport{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: controllers.leaderNamespace,
			Name:      getResourceExportName(controllers.localClusterID, nginxReq, "endpoints"),
		},
	}
	exportedSvcNginx := controllers.svcNginx.DeepCopy()

	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(exportedSvcNginx).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existSvcResExport, existEpResExport).Build()

	commonArea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader-cluster", controllers.localClusterID, "default", nil)
	mcReconciler := NewMemberClusterSetReconciler(fakeClient, controllers.scheme, "default", false)
	mcReconciler.SetRemoteCommonArea(commonArea)
	r := NewServiceExportReconciler(fakeClient, controllers.scheme, mcReconciler, "ClusterIP")
	r.installedSvcs.Add(&svcInfo{
		name:      controllers.svcNginx.Name,
		namespace: controllers.svcNginx.Namespace,
	})
	if _, err := r.Reconcile(controllers.ctx, nginxReq); err != nil {
		...
		err := fakeRemoteClient.Get(controllers.ctx, types.NamespacedName{
			Namespace: "default",
...
func TestServiceExportReconciler_CheckExportStatus(t *testing.T) {
	mcsSvc := controllers.svcNginx.DeepCopy()
	...
	nginx0Svc := controllers.svcNginx.DeepCopy()
	...
	nginx1Svc := controllers.svcNginx.DeepCopy()
	...
	nginx1EP := &corev1.Endpoints{
		...
		Subsets: controllers.epNginxSubset,
	}

	nginx2Svc := controllers.svcNginx.DeepCopy()
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(mcsSvc, nginx0Svc, nginx1Svc, nginx1EP, nginx2Svc, existSvcExport,
		nginx0SvcExport, nginx1SvcExportWithStatus, nginx2SvcExportWithStatus, mcsSvcExport).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).Build()
	commonArea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader-cluster", controllers.localClusterID, "default", nil)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := r.Reconcile(controllers.ctx, tt.req); err != nil {
				...
				err := fakeClient.Get(controllers.ctx, types.NamespacedName{Namespace: tt.req.Namespace, Name: tt.req.Name}, newSvcExport)
				if err != nil {
...
func TestServiceExportReconciler_handleServiceExportCreateEvent(t *testing.T) {
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(controllers.svcNginx, controllers.epNginx, existSvcExport).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).Build()
	...
	if _, err := r.Reconcile(controllers.ctx, nginxReq); err != nil {
		...
		err := fakeRemoteClient.Get(controllers.ctx, types.NamespacedName{Namespace: "default", Name: "cluster-a-default-nginx-service"}, svcResExport)
		if err != nil {
		...
		err = fakeRemoteClient.Get(controllers.ctx, types.NamespacedName{Namespace: "default", Name: "cluster-a-default-nginx-endpoints"}, epResExport)
		if err != nil {
			t.Errorf("ServiceExport Reconciler should get new Endpoints kind of ResourceExport successfully but got error = %v", err)
		}
		newSvcExport := &k8smcsv1alpha1.ServiceExport{}
		if err = fakeClient.Get(controllers.ctx, types.NamespacedName{Namespace: "default", Name: "nginx"}, newSvcExport); err != nil {
...
func TestServiceExportReconciler_handleUpdateEvent(t *testing.T) {
	sinfo := &svcInfo{
		name:       controllers.svcNginx.Name,
		namespace:  controllers.svcNginx.Namespace,
		clusterIPs: controllers.svcNginx.Spec.ClusterIPs,
		ports:      controllers.svcNginx.Spec.Ports,
		svcType:    string(controllers.svcNginx.Spec.Type),
	}

	epInfo := &epInfo{
		name:      controllers.epNginx.Name,
		namespace: controllers.epNginx.Namespace,
		subsets:   common.FilterEndpointSubsets(controllers.epNginx.Subsets),
	}

	newSvcNginx := controllers.svcNginx.DeepCopy()
	newSvcNginx.Spec.Ports = []corev1.ServicePort{controllers.svcPort8080}
	newEpNginx := controllers.epNginx.DeepCopy()
	newEpNginx.Subsets[0].Ports = controllers.epPorts8080
	...
	re := mcsv1alpha1.ResourceExport{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				common.SourceClusterID: controllers.localClusterID,
			},
		},
		Spec: mcsv1alpha1.ResourceExportSpec{
			ClusterID: controllers.localClusterID,
			Name:      nginxReq.Name,
	...
	existSvcRe.Spec.Service.ServiceSpec.Ports = []corev1.ServicePort{controllers.svcPort80}

	existEpRe := re.DeepCopy()
	existEpRe.Name = "cluster-a-default-nginx-endpoints"
	existEpRe.Spec.Endpoints = &mcsv1alpha1.EndpointsExport{Subsets: common.FilterEndpointSubsets(controllers.epNginxSubset)}
	...
	tests := []struct {
		...
		},
		{
			...
			existingEndpoints: svcNginxEPs,
			existingService:   controllers.svcNginx,
			endpointIPType:    "PodIP",
			expectedPorts: []corev1.ServicePort{
				{
					Name:     "http",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(tt.existingService, tt.existingEndpoints, existSvcExport).Build()
			fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existSvcRe, existEpRe).Build()

			commonArea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader-cluster", controllers.localClusterID, "default", nil)
			mcReconciler := NewMemberClusterSetReconciler(fakeClient, controllers.scheme, "default", false)
			mcReconciler.SetRemoteCommonArea(commonArea)
			r := NewServiceExportReconciler(fakeClient, controllers.scheme, mcReconciler, tt.endpointIPType)
