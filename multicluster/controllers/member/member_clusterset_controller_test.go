...
func TestMemberClusterDelete(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects().Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingMemberClusterAnnounce).Build()
	...
	if _, err := reconciler.Reconcile(controllers.ctx, reconcile.Request{
		...
		err := fakeClient.Get(controllers.ctx, types.NamespacedName{
			Namespace: "default",
			Name:      "member-announce-from-cluster-a",
...
func TestMemberClusterStatus(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingClusterSet).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingClusterSet).Build()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			commonArea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader1", controllers.localClusterID, "mcs1", tt.conditions)
			reconciler := MemberClusterSetReconciler{
				Client:           fakeClient,
				remoteCommonArea: commonArea,
...
func TestMemberCreateOrUpdateRemoteCommonArea(t *testing.T) {
	...
	fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingClusterSet, existingSecret).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(existingClusterSet, existingSecret).Build()
	commonArea := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader1", controllers.localClusterID, "mcs1", nil)
	reconciler := MemberClusterSetReconciler{
		Client:           fakeClient,
		remoteCommonArea: commonArea,
		clusterSetConfig: existingClusterSet,
