...
func TestGatewayReconciler(t *testing.T) {
	...
	for _, tt := range tests {
		...
		fakeClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(obj...).Build()
		fakeRemoteClient := fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects().Build()
		if tt.resExport != nil {
			fakeRemoteClient = fake.NewClientBuilder().WithScheme(controllers.scheme).WithObjects(tt.resExport).Build()
		}
		...
		r := NewGatewayReconciler(fakeClient, controllers.scheme, "default", "10.96.0.0/12", []string{"10.200.1.1/16"}, commonAreaGatter)
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := r.Reconcile(controllers.ctx, req); err != nil {
				...
				ciExportName := types.NamespacedName{
					Namespace: controllers.leaderNamespace,
					Name:      newClusterInfoResourceExportName(controllers.localClusterID),
				}
				err := fakeRemoteClient.Get(controllers.ctx, ciExportName, &ciExport)
...
func TestGetServiceCIDR(t *testing.T) {
	...
	r := NewGatewayReconciler(fakeClient, controllers.scheme, "default", "", []string{"10.200.1.1/16"}, nil)
