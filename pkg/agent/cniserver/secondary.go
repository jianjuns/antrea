...
func (pc *podConfigurator) ConfigureSriovSecondaryInterface(
	...
	if err = pc.ifConfigurator.advertiseContainerAddr(containerNetNS, containerIface.Name, result); err != nil {
		klog.ErrorS(err, "Failed to advertise IP address for SR-IOV interface", "containerID", containerID, "interface", containerInterfaceName)
	}
	return nil
}
...
func (pc *podConfigurator) ConfigureVLANSecondaryInterface(
	...
	defer func() {
		if !success {
			if err := pc.ifConfigurator.removeContainerLink(containerID, hostIface.Name); err != nil {
				klog.ErrorS(err, "failed to roll back veth creation", "containerID", containerID, "interface", containerInterfaceName)
			}
		}
	}()
	...
	klog.InfoS("Configured VLAN interface", "Pod", podName, "Namespace", podNamespace, "interface", containerInterfaceName, "hostInterface", hostIface)

	if err := pc.ifConfigurator.advertiseContainerAddr(containerNetNS, containerIface.Name, result); err != nil {
		klog.ErrorS(err, "Failed to advertise IP address for VLAN interface", "containerID", containerID, "interface", containerInterfaceName)
	}
	success = true
