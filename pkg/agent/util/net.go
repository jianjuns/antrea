...
// Pod's Name, container ID, and the inner veth interface name. The output
// should be deterministic.
func GenerateContainerOuterVethName(podName, podNamespace, containerID, innerVeth string) string {
	var key string
	if innerVeth == "eth0" {
		key = containerID
	} else {
		// Secondary interface.
		key = containerID + innerVeth
	}
